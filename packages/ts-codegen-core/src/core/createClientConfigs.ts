...
const buildConfigs = <TOperation extends CustomOperation>({
  ...
  const createConfig = (path: CustomPath, pathName: string): IClientConfig[] => {
    ...
    return keys(operations).map((method) => {
      ...
      const __basePath = basePath.replace(/^\//, '');
      const serviceId = __basePath.split('-').map(item => titleCase(item)).join('');
      const requestConfigId = pathName.split('/').map(item =>  titleCase(item)).join('');
      const __curName = serviceId + requestConfigId +  titleCase(method);
      const __operationId = __curName.replace(/{[^}]+}/g, 'Tag');
      const params = {
  ...
  const xxx = reduce(
    ...
    (configs: IClientConfig[], path: CustomPath, pathName: string) => {
      return [...configs, ...createConfig(path, pathName)]
    },
    [],
  )
  return xxx;
